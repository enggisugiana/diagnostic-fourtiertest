...
import { Question, QuizAttempt, Student, TierAnswer, CategoryStats, SubjectResult, DiagnosticRule } from '@/types';
...
const StudentPortal: React.FC<StudentPortalProps> = ({ 
  ...
  useEffect(() => {
    // Check if student already has a finished attempt for this session
    ...
    if (existingAttempt) {
      ...
      setActiveQuiz(null); // Clear any partial quiz state if a full attempt exists
      return;
    }
  ...
  const calculateStatsForSubset = (subsetQuestions: Question[], subsetAnswers: TierAnswer[]) => {
    ...
    subsetAnswers.forEach((ans, idx) => {
      ...
      // Hardcoded 16-rule logic to ensure compliance with user's table
      if (t1Correct && t3Correct && t2Sure && t4Sure) {
        ...
        // Everything else where both have been answered is Paham Sebagian
        stats.pahamSebagian++;
      }
    });
  ...
  const finishQuiz = (quizState: NonNullable<typeof activeQuiz>) => {
    try {
      ...
      const subjectResults: SubjectResult[] = Object.entries(subjectGroups).map(([subject, indicatorsDict]) => {
        let subjectPoints = 0;
        let subjectQs = 0;
        const subjectStats: CategoryStats = {
      ...
      // CRITICAL: Update local state FIRST to show results screen
  ...
  const getPerc = (val: number, total: number) => {
    if (total === 0) return 0;
  ...
  if (finishedAttempt) {
    ...
    // Safety check and parsing for JSON fields that might be strings if backend returned them raw
    try {
    ...
    const categoryRules = [
      { id: 1, cat: 'pahamKonsep', t1: true, t2: true, t3: true, t4: true },
      { id: 2, cat: 'pahamSebagian', t1: true, t2: true, t3: true, t4: false },
      { id: 3, cat: 'pahamSebagian', t1: true, t2: false, t3: true, t4: true },
      { id: 4, cat: 'pahamSebagian', t1: true, t2: false, t3: true, t4: false },
      { id: 5, cat: 'pahamSebagian', t1: true, t2: true, t3: false, t4: true },
      { id: 6, cat: 'pahamSebagian', t1: true, t2: true, t3: false, t4: false },
      { id: 7, cat: 'pahamSebagian', t1: true, t2: false, t3: false, t4: true },
      { id: 8, cat: 'pahamSebagian', t1: true, t2: false, t3: false, t4: false },
      { id: 9, cat: 'pahamSebagian', t1: false, t2: true, t3: true, t4: true },
      { id: 10, cat: 'pahamSebagian', t1: false, t2: true, t3: true, t4: false },
      { id: 11, cat: 'pahamSebagian', t1: false, t2: false, t3: true, t4: true },
      { id: 12, cat: 'pahamSebagian', t1: false, t2: false, t3: true, t4: false },
      { id: 13, cat: 'miskonsepsi', t1: false, t2: true, t3: false, t4: true },
      { id: 14, cat: 'tidakPahamKonsep', t1: false, t2: true, t3: false, t4: false },
      { id: 15, cat: 'tidakPahamKonsep', t1: false, t2: false, t3: false, t4: true },
      { id: 16, cat: 'tidakPahamKonsep', t1: false, t2: false, t3: false, t4: false }
    ];

    const getCategoryDetails = (cat: string) => {
      switch(cat) {
        case 'pahamKonsep': return { name: 'Paham Konsep', color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' };
        case 'pahamSebagian': return { name: 'Paham Sebagian', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' };
        case 'miskonsepsi': return { name: 'Miskonsepsi', color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' };
        case 'tidakPahamKonsep': return { name: 'Tidak Paham Konsep', color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100' };
        default: return { name: 'Tidak Dikategorikan', color: 'text-slate-500', bg: 'bg-slate-50', border: 'border-slate-100' };
      }
    };
