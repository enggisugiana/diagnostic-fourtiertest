...
export default function AddQuestionPage() {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await questionService.create(submittedData as any);
      router.push(`/console/management/question-list/${indicatorId}?subjectId=${subjectId}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 animate-fadeIn">
      <div className="flex justify-between items-center mb-8 border-b border-teal-50 pb-6">
        <div>
           <h2 className="text-2xl font-black text-[#016569] uppercase tracking-tight">Tambah Soal 4-Tier</h2>
           <p className="text-teal-400 text-xs font-black uppercase tracking-widest mt-1">Konfigurasi Pertanyaan & Alasan</p>
        </div>
        <Link 
          href={`/console/management/question-list/${indicatorId}?subjectId=${subjectId}`} 
      ...
      <form onSubmit={handleManualSubmit} className="space-y-8 pb-20">
        ...
        <button disabled={loading} type="submit" className="w-full bg-[#016569] text-white py-5 rounded-2xl font-black text-sm shadow-2xl hover:bg-[#015255] hover:-translate-y-1 transition-all disabled:opacity-50 flex items-center justify-center gap-3">
          {loading ? (
            <div className="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
          ) : (
            <i className="fas fa-save text-lg"></i>
          )}
          <span>{loading ? 'MENYIMPAN...' : 'SIMPAN SEMUA DATA SOAL'}</span>
        </button>
      </form>
    </div>
  );
